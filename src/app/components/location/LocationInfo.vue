<template>
  <div class="rounded-lg bg-white p-6 shadow-md">
    <h2 class="mb-4 text-2xl font-bold text-gray-900">{{ location.name }}</h2>
    ...
    <!-- Rating -->
    <div v-if="location.averageRating" class="mb-4 flex items-center gap-2">
      <Rating :model-value="location.averageRating" :readonly="true" />
      <span class="text-gray-600">
        {{ formatRating(location.averageRating) }} ({{ location._count?.reviews || 0 }}
        {{ pluralize(location._count?.reviews || 0, 'review') }})
      </span>
    </div>

    <!-- Description -->
    <p v-if="location.description" class="mb-6 text-gray-700">
